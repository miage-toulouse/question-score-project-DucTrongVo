...
public class QuestionAChoixMultiple implements QuestionAChoix {
     ...
     * @param totalNbOfChoices le nombre total de choix pour la question.
     */
    public QuestionAChoixMultiple(String enonce, List<Integer> indicesBonnesReponses, int totalNbOfChoices) {
        this.enonce = enonce;
        this.indicesBonnesReponses = indicesBonnesReponses;
        this.totalNbOfChoices = totalNbOfChoices;
    }
    ...
    public float getScoreForIndice(int indiceChoix) {
        if (indicesBonnesReponses.contains(indiceChoix)) {
            ...
            return -100f/((float)(this.totalNbOfChoices - indicesBonnesReponses.size()));
        }
